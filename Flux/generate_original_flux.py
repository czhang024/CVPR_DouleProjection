"""
generate_original_flux.py
Generate baseline images using the original FLUX.1-schnell model.
Mirrors the structure of SD/generate_original_sd.py.
"""
...
class OriginalFluxGenerationPipeline:
    def __init__(self,
                 model_id='black-forest-labs/FLUX.1-schnell',
                 device='cuda',
                 output_dir='generated_original_flux_schnell',
                 resolution=512,
                 batch_size=1,
        ...
        print(f"Resolution: {resolution}x{resolution}  |  Batch size: {batch_size}  |  Steps: {num_inference_steps}")
        ...
        # Memory optimisations — keep or remove depending on VRAM
    ...
    def generate_images_from_prompts(self, prompts_data, objects_prompts,
        ...
        # Save prompt list
        # Logs go into a separate subfolder so the images folder stays clean
    ...
    def _save_log(self, generation_log):
        ...
        with open(report_path, 'w', encoding='utf-8') as fh:
            ...
            fh.write("=" * 60 + "\n\n")
            fh.write(f"Model: {self.model_id} (original FLUX.1-schnell)\n")
...
def main():
    parser = argparse.ArgumentParser(description='Generate images using original FLUX.1-schnell model')
    parser.add_argument('--prompt_csv', type=str, default='../SD/data/small_imagenet_prompts.csv')
    parser.add_argument('--output_dir', type=str, default='generated_original_flux')
    ...
    pipeline = OriginalFluxGenerationPipeline(
    ...
    print(f"\n{'='*60}")
    print(f"FLUX.1-schnell baseline generation")
