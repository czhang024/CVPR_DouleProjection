...
# "french horn" → tokens ['french', 'horn'] in both CLIP and T5.
CUDA_VISIBLE_DEVICES=0 python Double_proxy.py \
    ...
    --concept_type "object" \
    --save_dir "./models" \
    --exp_name "flux_double_proxy_FrenchHorn"
...
CUDA_VISIBLE_DEVICES=0 python generate_erased_flux.py \
    --erased_model models/flux_double_proxy_FrenchHorn.safetensors \
    --output_dir generated_Double_FrenchHorn_flux \
...
CUDA_VISIBLE_DEVICES=0 python compare_accuracy.py \
    ...
    --erased_path generated_Double_FrenchHorn_flux \
    --concept 'french horn' \
    --prompts_csv '../SD/data/small_imagenet_prompts.csv' \
    --output_log 'DoubleResult_FrenchHorn_flux.log'
