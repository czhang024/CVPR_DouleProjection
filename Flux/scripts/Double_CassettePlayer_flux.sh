...
# Double Proxy for erasing cassette player while preserving other ImageNette concepts
CUDA_VISIBLE_DEVICES=0 python Double_proxy.py \
    ...
    --concept_type "object" \
    --save_dir "./models" \
    --exp_name "flux_double_proxy_CassettePlayer"
...
CUDA_VISIBLE_DEVICES=0 python generate_erased_flux.py \
    --erased_model models/flux_double_proxy_CassettePlayer.safetensors \
    --output_dir generated_Double_CassettePlayer_flux \
...
CUDA_VISIBLE_DEVICES=0 python compare_accuracy.py \
    ...
    --erased_path generated_Double_CassettePlayer_flux \
    --concept 'cassette player' \
    --prompts_csv '../SD/data/small_imagenet_prompts.csv' \
    --output_log 'DoubleResult_CassettePlayer_flux.log'
