...
# UCE Original for erasing chain saw concept while preserving other ImageNette concepts
CUDA_VISIBLE_DEVICES=1 python UCE_original.py \
    ...
    --erase_scale 1.0 \
    --save_dir "./models" \
    --exp_name "UCE_original_ChainSaw"
...
CUDA_VISIBLE_DEVICES=1 python generate_erased_small_imagenet.py \
    --erased_model models/UCE_original_ChainSaw/diffusion_pytorch_model.safetensors \
    --output_dir generated_UCE_ChainSaw \
    --num_variations 10
...
CUDA_VISIBLE_DEVICES=1 python compare_accuracy.py \
    ...
    --erased_path generated_UCE_ChainSaw \
    --concept 'chain saw' \
    --prompts_csv 'data/small_imagenet_prompts.csv' \
    --output_log 'UCEResult_ChainSaw.log'
