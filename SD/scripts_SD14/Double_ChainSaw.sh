...
# Double Proxy UCE for erasing chain saw concept while preserving other ImageNette concepts
CUDA_VISIBLE_DEVICES=0 python Double_proxy.py \
    ...
    --concept_type "object" \
    --save_dir "./models" \
    --exp_name "UCE_double_proxy_ChainSaw"
...
CUDA_VISIBLE_DEVICES=0 python generate_erased_small_imagenet.py \
    --erased_model models/UCE_double_proxy_ChainSaw/diffusion_pytorch_model.safetensors \
    --output_dir generated_Double_ChainSaw \
    --num_variations 10
...
CUDA_VISIBLE_DEVICES=0 python compare_accuracy.py \
    ...
    --erased_path generated_Double_ChainSaw \
    --concept 'chain saw' \
    --prompts_csv 'data/small_imagenet_prompts.csv' \
    --output_log 'DoubleResult_ChainSaw.log'
