...
# Double Proxy UCE for erasing golf ball concept while preserving other ImageNette concepts
CUDA_VISIBLE_DEVICES=0 python Double_proxy.py \
    ...
    --concept_type "object" \
    --save_dir "./models" \
    --exp_name "UCE_double_proxy_GolfBall"
...
CUDA_VISIBLE_DEVICES=0 python generate_erased_small_imagenet.py \
    --erased_model models/UCE_double_proxy_GolfBall/diffusion_pytorch_model.safetensors \
    --output_dir generated_Double_GolfBall \
    --num_variations 10
...
CUDA_VISIBLE_DEVICES=0 python compare_accuracy.py \
    ...
    --erased_path generated_Double_GolfBall \
    --concept 'golf ball' \
    --prompts_csv 'data/small_imagenet_prompts.csv' \
    --output_log 'DoubleResult_GolfBall.log'
